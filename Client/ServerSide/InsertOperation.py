...
class InsertOperation():
    ...
    def insertAttendance(self, total_student, present_student, course_id, year, teacher_id, subject_id, date):
        try:
            for i in range(0, len(total_student)):
                ...
                for j in range(0, len(present_student)):
                    if total_student[i][0] == present_student[j]:
                        query = "INSERT INTO Attendance(date, student_id, present, absent, year, subject_id, course_id, teacher_id) values(" \
                                "%s, %s, 'Y', 'N', %s, %s, %s, %s)"
                        value = [date, present_student[j], year, subject_id, course_id, teacher_id]
                ...
                if not present:
                    query = "INSERT INTO Attendance(date, student_id, present, absent, year, subject_id, course_id, teacher_id) values(" \
                            "%s, %s, 'N', 'Y', %s, %s, %s, %s)"
                    value = [date, total_student[i][0], year, subject_id, course_id, teacher_id]
                    self.cur.execute(query, value)
                    self.conn.commit()
            return True
        except:
